...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const sidebarBg = isPrimary ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-stone-200';
  const itemHover = isPrimary ? 'hover:bg-white hover:text-indigo-600 hover:shadow-md' : 'hover:bg-stone-50 hover:text-stone-800';
  const itemActive = isPrimary ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-200 ring-0' : 'bg-teal-50 text-teal-700 font-semibold shadow-sm ring-1 ring-teal-100';
  const iconActive = isPrimary ? 'text-white' : 'text-teal-600';
          ...
          return (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 group mb-1 ${isActive
                ? itemActive
                : `text-stone-500 ${itemHover}`
                }`}
            >
              <item.icon className={`w-5 h-5 transition-colors ${isActive ? iconActive : 'text-stone-400 group-hover:text-stone-600'}`} />
