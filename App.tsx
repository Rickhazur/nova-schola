...
const App: React.FC = () => {
        ...
        return (
            <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 relative overflow-hidden font-sans">
                ...
                <div className="relative z-10 w-full max-w-md">
                    ...
                    {/* LEVEL SELECTION FOR DEMO */}
                    <div className="flex justify-center gap-2 mb-4">
                        <button onClick={() => setLoginLevel('primary')} className={`px-3 py-1 rounded-lg text-xs font-bold border ${loginLevel === 'primary' ? 'bg-cyan-600 border-cyan-500 text-white' : 'border-white/10 text-slate-400'}`}>Primaria</button>
                        <button onClick={() => setLoginLevel('bachillerato')} className={`px-3 py-1 rounded-lg text-xs font-bold border ${loginLevel === 'bachillerato' ? 'bg-indigo-600 border-indigo-500 text-white' : 'border-white/10 text-slate-400'}`}>Bachillerato</button>
                    </div>
    ...
    return (
        <div className="flex bg-white min-h-screen font-sans text-slate-900 overflow-hidden">
            ...
            <main className={`flex-1 overflow-y-auto h-screen relative md:ml-64 ${userLevel === 'primary' ? 'bg-indigo-50/50' : 'bg-stone-50/30'}`}>
